...
export KVS_LIB="$LIB_FILE:Kvs:${KVMS_ADDR}"
...
export CONTRACT_ADDR=`forge create --legacy --rpc-url $RPC_URL --private-key $KEY ./src/Factory.sol:Factory --libraries $UTILS_LIB --libraries $KVS_LIB --libraries $STR_ADDRS_LIB --libraries $TAGS_LIB |grep "Deployed to:"|sed "s/Deployed to: //"`
